...
history = []
warehouse = {
    "bike": {"price": 540, "quantity": 0},
    "computer": {"price": 4800, "quantity": 3},
    "smartphone": {"price": 2700, "quantity": 8},
    "microwave": {"price": 1100, "quantity": 3},
}

# Loading data from files on startup
try:
    with open(BALANCE_FILE, "r") as f:
        company_balance = int(f.readline())
    print(f"Balance loaded from {BALANCE_FILE}: {company_balance} PLN")
...
try:
    with open(WAREHOUSE_FILE, "r") as f:
        ...
        if warehouse_data:
            warehouse = literal_eval(warehouse_data)
...
try:
    ...
    print(f"History loaded from {HISTORY_FILE}. Found {len(history)} operations.")
except FileNotFoundError:
    ...
    #Exiting the program
        ...
        try:
            with open(BALANCE_FILE, "w") as f:
                f.write(str(company_balance))
            print(f"Balance saved in {BALANCE_FILE}: {company_balance} PLN")
